...
export async function getLatestAlarm(): Promise<any> {
    
    const apiUrl = await getServerURL();
    const connectionString = `${apiUrl}/api/alarms/latestAlarm`;
    ...
    try {
        ...
        console.error("Fetch or parsing error while performing call (getLatestAlarm.ts):", error);
